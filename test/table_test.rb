...
class TestTableFormattingHooks < Test::Unit::TestCase
  ...
  def test_to_hack
    table = Ruport::Data::Table.new :column_names => %w[a b], 
                                    :data => [[1,2],[3,4],[5,6]]
    assert_equal("a,b\n1,2\n3,4\n5,6\n",table.to_csv)
    assert_raises(Ruport::Report::UnknownFormatError) { table.to_nothing }
  end
  ...
  def test_as_throws_proper_errors
    ...
    assert_raises(Ruport::Report::UnknownFormatError) { a.as(:nothing) }
    assert_raises(Ruport::Report::UnknownFormatError) { a.to_nothing }
  end
